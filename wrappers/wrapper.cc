...
int clang_wrapper(vector<string> args) {
    ...
    for (string arg: c_file_arr){
        ...
        if (!print) {
            ...
            //std::thread new_thread([](char* const* to_run){execv(to_run[0], to_run);}, &thread_args[0]);
            pid_t cpid;
            if((cpid = fork()) == 0){
                // char*const to_run[] = &thread_args[0];
                // argument of type "const char **" is incompatible with parameter of type "char *const *"C/C++(167)
