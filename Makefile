...
COMMON_CXXFLAGS := $(COMMON_CFLAGS) --std=c++17 -Ideps/CLI11/include -Ideps/fmt/include -DFMT_HEADER_ONLY
...
debug: $(DEBUG_DIR)/bin/rkr \
			 ...
			 $(DEBUG_WRAPPERS) \
			 $(DEBUG_DIR)/share/rkr/ssh-wrapper
...
release: $(RELEASE_DIR)/bin/rkr \
         ...
         $(RELEASE_WRAPPERS) \
	 $(RELEASE_DIR)/share/rkr/ssh-wrapper
...
$(DEBUG_DIR)/share/rkr/ssh-wrapper $(RELEASE_DIR)/share/rkr/ssh-wrapper: src/ssh-wrapper/ssh-wrapper.cc Makefile
	@mkdir -p `dirname $@`
