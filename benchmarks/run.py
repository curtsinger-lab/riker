...
class Config:
    def __init__(self, pargs: argparse.Namespace, confpath: str, data: Dict[str, str]):
        ...
        self.benchmark_path: str = os.path.abspath(
            os.path.dirname(os.path.realpath(confpath))
        )
        # run outside of docker?
        self.no_docker = pargs.no_docker
        # location of benchmark dir in Docker
        self.benchmark_root: str = (
            os.path.abspath(data["no_docker_root"])
            if self.no_docker
            else data["benchmark_root"]
        )
        ...
        if "no_docker_init_script" in data:
            self.no_docker_init_script = os.path.join(
                self.benchmark_path, data["no_docker_init_script"]
            )
        elif self.no_docker:
            raise Exception(
    ...
    def docker_rm_file_cmd(
        self, docker_file: str, ignore_failure: bool, recursive: bool
    ) -> List[str]:
        flags: str = ""
    ...
    def no_docker_init_cmd(self) -> List[str]:
        return [
            self.no_docker_init_script,  # path to init script
    ...
    def image_is_initialized(self) -> bool:
        if self.no_docker:
            ...
            rc: int
            rv: str
            rc, rv = run_command_capture(
                self.docker_images_cmd(), suppress_printing=False
            )
            if rc == 0:
                first: bool = True
                for line in rv.splitlines():
                    ...
                    if (
                        m
                        and (m.group("repository") == self.docker_image_name())
                        and (m.group("tag") == self.docker_image_version())
                    ):
                        return True
                return False
            else:
                print(
                    "ERROR: Unable to query docker ({}) for image data (return code: {}).".format(
                        self.docker_exe, rc
                    )
                )
                sys.exit(1)
    ...
    def container_is_running(self) -> bool:
        if self.no_docker:
            ...
            if not self.image_is_initialized():
                print(
                    "ERROR: Docker image '{}' is not initialized".format(
                        self.docker_image_fullname()
                    )
                )
                sys.exit(1)
            rc: int
            ...
            if rc == 0:
                ...
                for line in rv.splitlines():
                    ...
                    m: Optional[Match[str]] = p.search(line)
                    if m and (
                        m.group("container_name") == self.docker_container_name()
                    ):
                        return True
                return False
            else:
                print(
                    "ERROR: Unable to query docker ({}) for container data (return code: {}).".format(
                        self.docker_exe, rc
                    )
                )
                sys.exit(1)
    ...
    def container_is_dead(self) -> bool:
        if self.no_docker:
            ...
            rv: str
            rc, rv = run_command_capture(
                self.docker_containers_cmd(["status=dead", "status=exited"])
            )
            if rc == 0:
                first: bool = True
                for line in rv.splitlines():
    ...
    def benchmark_remove(
        self, no_docker: bool, rm_image: bool, ignore_failure: bool = False
    ) -> int:
        if no_docker:
            ...
            rc, _ = run_command_capture(
                conf.docker_stop_container_cmd(), suppress_printing=ignore_failure
            )
            if rc != 0 and not ignore_failure:
                print(
                    "ERROR: Unable to stop container '{}'.".format(
            ...
            rc, _ = run_command_capture(
                conf.docker_rm_container_cmd(), suppress_printing=ignore_failure
            )
            if rc != 0 and not ignore_failure:
                print(
                    "ERROR: Unable to remove stopped container '{}'".format(
                        conf.docker_container_name()
                    )
                )
                # don't die; see above
            ...
            if rm_image:
                rc, _ = run_command_capture(
                    conf.docker_rm_image_cmd(), suppress_printing=ignore_failure
                )
                if rc != 0 and not ignore_failure:
                    print(
                        "ERROR: Unable to remove image '{}'".format(
                            conf.docker_image_fullname()
                        )
                    )
                    # again, don't die
    ...
    def docker_rm_file(self, path: str, ignore_failure: bool, recursive: bool) -> int:
        ...
        if rc != 0:
            print(
                "ERROR: Unable to remove file '{}' in docker container '{}'.".format(
                    path, conf.docker_container_name()
                )
            )
        return rc
    ...
    def docker_stats(self) -> DockerStats:
        ...
        if rc != 0:
            print(
                "ERROR: Unable to obtain docker stats for docker container '{}'.".format(
                    conf.docker_container_name()
                )
            )
            sys.exit(1)
...
def init_configs(args: List[str]) -> Configs:
    parser = argparse.ArgumentParser(
        description="Runs a benchmark given at least one JSON "
    ...
    parser.add_argument(
        "--make", help="measure clean build baseline using make", action="store_true"
    )
    parser.add_argument(
        "--no-docker",
    ...
    parser.add_argument(
        "--no-bench",
        help="Don't run the benchmark, just do setup/cleanup actions",
        action="store_true",
    )
...
def run_command_capture(
    command: List[str], env: Dict[str, str] = {}, suppress_printing: bool = True
) -> Tuple[int, str]:
    ...
    if rc is not None:
        ...
        raise Exception(
            "Cannot obtain return code for command '{}'.".format(command[0])
        )
...
def prepend_column(
    header: str, column: List[str], csv_header: str, csv_rows: List[str]
) -> CSV:
...
def run_benchmark(conf: Config, tool: Tool, task: Task) -> CSV:
    ...
    # add benchmark name
    header, rows = prepend_column(
        "benchmark_name", [conf.benchmark_name] * nrows, header, rows
    )

    # add docker mode
    header, rows = prepend_column(
        "docker_mode", ["TRUE" if not conf.no_docker else "FALSE"] * nrows, header, rows
    )
...
def run_suite(conf: Config, tool: Tool, rebuild: bool, needs_cleanup: bool) -> None:
    ...
    if needs_cleanup:
        rc: int = conf.benchmark_remove(
            no_docker=conf.no_docker, rm_image=False, ignore_failure=False
        )
        if rc != 0:
            print(
                "Unable to reset docker configuration to run '{}' benchmark.".format(
                    conf.benchmark_name
                )
            )
            sys.exit(1)
    ...
    if conf.container_is_dead():
        if not conf.no_docker:
            print(
                "INFO: Docker container '{}' is dead.  Removing old container and restarting...".format(
                    conf.docker_container_name()
                )
            )
        ...
        if not conf.no_docker:
            print(
                "INFO: Docker container '{}' is not running.  Starting...".format(
                    conf.docker_container_name()
                )
            )
        conf.start_container()
        ...
        if not conf.no_docker:
            print(
                "INFO: Docker container '{}' is already running.  Skipping startup.".format(
                    conf.docker_container_name()
                )
            )
...
for conf in c.configs:
    ...
    if conf.cleanup_before:
        ...
        conf.benchmark_remove(
            no_docker=conf.no_docker, rm_image=True, ignore_failure=True
        )
    ...
    if not conf.image_is_initialized():
        if conf.no_docker:
            print(
                "INFO: No-docker benchmark '{}' is not initialized.  Initializing...".format(
                    conf.benchmark_name
                )
            )
        else:
            print(
                "INFO: Docker image '{}' is not initialized.  Initializing...".format(
                    conf.docker_image_fullname()
                )
            )
        conf.initialize_docker_image()
        ...
        print(
            "INFO: Docker image '{}' is already initialized.  Skipping initialization.".format(
                conf.docker_image_fullname()
            )
        )
