...
const unique_ptr<Run>& Command::currentRun() noexcept {
  if (!_run) _run = make_unique<Run>(shared_from_this());
  return _run;
}

// Get the command run data for the previous run
const unique_ptr<Run>& Command::previousRun() noexcept {
  if (!_last_run) _last_run = make_unique<Run>(shared_from_this());
  return _last_run;
}
...
void Command::finishRun() noexcept {
  ...
  _last_run = make_unique<Run>(shared_from_this());
