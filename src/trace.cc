...
void trace_step(Trace& trace, pid_t child, int wait_status) {
    ...
    case STOP_SYSCALL: {
        ...
        case /* 257 */ __NR_openat: {
            ...
            trace.add_open(child, registers.SYSCALL_RETURN, main_file, access_mode, rewrite, cloexec, mode);
            break;
        }
        case /* 22 */ __NR_pipe:
        ...
        case /* 33 */ __NR_dup2:
            ...
            break;
        case /* 292 */ __NR_dup3:
            trace.add_dup(child, registers.SYSCALL_ARG1, registers.SYSCALL_ARG2, (registers.SYSCALL_ARG3 & O_CLOEXEC) != 0);
            break;
        case /* 72 */ __NR_fcntl:
            switch (registers.SYSCALL_ARG2) {
            case F_DUPFD:
            ...
            case F_DUPFD_CLOEXEC:
                ...
                break;
            case F_SETFD:
                trace.add_set_cloexec(child, registers.SYSCALL_ARG1, (registers.SYSCALL_ARG3 & FD_CLOEXEC) != 0);
                break;
            }
            break;
