...
struct CommandRecord : public Record {
  ...
  CommandRecord(Command::ID id,
                ...
                std::vector<Command::diff> envDiffs,
                std::map<int, Ref::ID> initial_fds,
                bool executed) :
      _id(id), _args(args), _envDiffs(envDiffs), _initial_fds(initial_fds), _executed(executed) {}
  ...
  static std::unique_ptr<Record> create(Command::ID id,
                                        ...
                                        std::map<int, Ref::ID> initial_fds,
                                        bool executed) {
    return std::make_unique<CommandRecord>(id, args, envDiffs, initial_fds, executed);
  }
  ...
  template <class Archive>
  void serialize(Archive& archive) {
    archive(cereal::base_class<Record>(this), _id, _args, _envDiffs, _initial_fds, _executed);
  }
};
