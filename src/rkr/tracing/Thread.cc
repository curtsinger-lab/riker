...
void Thread::syscallEntryChannel(Build& build, ssize_t channel) noexcept {
...
Ref::ID Thread::makePathRef(Build& build, fs::path p, AccessFlags flags, at_fd at) noexcept {
...
void Thread::_openat(Build& build,
  ...
  auto ref_id = makePathRef(build, filename, ref_flags, dfd);
...
void Thread::_mknodat(Build& build,
  ...
  if (mode.isRegularFile()) {
    ...
    _openat(build, dfd, filename, o_flags(O_CREAT | O_EXCL), mode);
...
void Thread::_close(Build& build, int fd) noexcept {
...
void Thread::_dup(Build& build, int fd) noexcept {
...
void Thread::_dup3(Build& build, int oldfd, int newfd, o_flags flags) noexcept {
...
void Thread::_fcntl(Build& build, int fd, int cmd, unsigned long arg) noexcept {
  LOGF(trace, "{}: fcntl({}, {}, {})", this, fd, cmd, arg);

  if (cmd == F_DUPFD) {
    // Handle fcntl(F_DUPFD) as a dup call. The return value is the new fd.
    _dup(build, fd);  // _dup will resume the process and return the new fd to us
    ...
    _dup3(build, fd, -1, o_flags(O_CLOEXEC));
...
void Thread::_fchown(Build& build, int fd, uid_t user, gid_t group) noexcept {
...
void Thread::_fchmod(Build& build, int fd, mode_flags mode) noexcept {
...
void Thread::_read(Build& build, int fd) noexcept {
...
void Thread::_write(Build& build, int fd) noexcept {
...
void Thread::_truncate(Build& build, fs::path pathname, long length) noexcept {
  LOGF(trace, "{}: truncate({}, {})", this, pathname, length);

  // Make an access to the reference that will be truncated
  auto ref_id = makePathRef(build, pathname, WriteAccess);
  const auto& ref = getCommand()->getRef(ref_id);
...
void Thread::_ftruncate(Build& build, int fd, long length) noexcept {
...
void Thread::_tee(Build& build, int fd_in, int fd_out) noexcept {
...
void Thread::_mkdirat(Build& build, at_fd dfd, fs::path pathname, mode_flags mode) noexcept {
  ...
  auto parent_ref = makePathRef(build, parent_path, WriteAccess, dfd);
...
void Thread::_renameat2(Build& build,
  ...
  auto old_dir_ref = makePathRef(build, old_dir, WriteAccess, old_dfd);

  auto old_entry_ref = makePathRef(build, old_path, NoFollowAccess, old_dfd);
  ...
  auto new_dir_ref = makePathRef(build, new_dir, WriteAccess, new_dfd);
...
void Thread::_getdents(Build& build, int fd) noexcept {
...
void Thread::_linkat(Build& build,
  ...
  auto dir_ref = makePathRef(build, dir_path, WriteAccess, new_dfd);

  // Get a reference to the link we are creating
  auto entry_ref = makePathRef(build, newpath, NoAccess, new_dfd);
...
void Thread::_symlinkat(Build& build, fs::path target, at_fd dfd, fs::path newpath) noexcept {
  ...
  // Get a reference to the directory, which we will be writing
  auto dir_ref = makePathRef(build, dir_path, WriteAccess, dfd);
...
void Thread::_readlinkat(Build& build, at_fd dfd, fs::path pathname) noexcept {
  ...
  auto ref_id = makePathRef(build, pathname, SymlinkAccess + NoFollowAccess, dfd);
  const auto& ref = getCommand()->getRef(ref_id);
...
void Thread::_unlinkat(Build& build, at_fd dfd, fs::path pathname, at_flags flags) noexcept {
  ...
  // Get a reference to the directory, which we will be writing
  auto dir_ref_id = makePathRef(build, dir_path, WriteAccess, dfd);

  // Get a reference to the entry itself
  auto ref_flags = NoFollowAccess + (flags.removedir() ? DirAccess : NotDirAccess);
  auto entry_ref_id = makePathRef(build, pathname, ref_flags, dfd);
...
void Thread::_umask(Build& build, mode_t mask) noexcept {
...
void Thread::_chdir(Build& build, fs::path filename) noexcept {
...
void Thread::_chroot(Build& build, fs::path filename) noexcept {
  LOGF(trace, "{}: chroot({})", this, filename);
  FAIL << "Builds that use chroot are not supported.";
}

void Thread::_pivot_root(Build& build, fs::path new_root, fs::path put_old) noexcept {
...
void Thread::_execveat(Build& build, at_fd dfd, fs::path filename, vector<string> args) noexcept {
  ...
  if (parent_exe.parent_path().filename() == "wrappers") {
    finishSyscall([=](Build& b, long rc) { resume(); });
    return;
  }

  // The parent command needs execute access to the exec-ed path
  auto exe_ref_id = makePathRef(build, filename, ExecAccess, dfd);
