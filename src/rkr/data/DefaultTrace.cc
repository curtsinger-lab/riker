...
void DefaultTrace::sendTo(IRSink& handler) noexcept {
  ...
  handler.specialRef(SavedIRSource(), _root_command, SpecialRef::stdin, Ref::Stdin);
  handler.usingRef(SavedIRSource(), _root_command, Ref::Stdin);

  // Create a reference to stdout
  handler.specialRef(SavedIRSource(), _root_command, SpecialRef::stdout, Ref::Stdout);
  handler.usingRef(SavedIRSource(), _root_command, Ref::Stdout);

  // Create a reference to stderr
  handler.specialRef(SavedIRSource(), _root_command, SpecialRef::stderr, Ref::Stderr);
  handler.usingRef(SavedIRSource(), _root_command, Ref::Stderr);

  // Set up the reference to the root directory
  handler.specialRef(SavedIRSource(), _root_command, SpecialRef::root, Ref::Root);
  handler.usingRef(SavedIRSource(), _root_command, Ref::Root);

  // Set up the reference to the working directory
  handler.specialRef(SavedIRSource(), _root_command, SpecialRef::cwd, Ref::Cwd);
  handler.usingRef(SavedIRSource(), _root_command, Ref::Cwd);

  // Set up the reference to the launch executable
  handler.specialRef(SavedIRSource(), _root_command, SpecialRef::launch_exe, Ref::Exe);
  handler.usingRef(SavedIRSource(), _root_command, Ref::Exe);
  ...
  handler.launch(SavedIRSource(), _root_command, rkr_build_cmd, refs);
